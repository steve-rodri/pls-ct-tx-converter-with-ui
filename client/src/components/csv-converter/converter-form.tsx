  ...
  const convertMutation = useMutation({
    ...
    onSuccess: async (response) => {
      try {
        ...
        const filename = response.headers.get("Content-Disposition")?.split("filename=")[1]?.replace(/"/g, "") || "converted.csv";
    ...
    onError: (error: any) => {
      ...
      setErrorMessage(error.message || "Failed to process CSV file. Please try again.");
      setShowErrorNotification(true);
    },
  });
